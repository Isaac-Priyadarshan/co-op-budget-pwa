...
const METALS: {
  id: Metal; emoji: string
  accent: string; glow: string; bg: string
  border: string; selBorder: string; gradient: string
}[] = [
...
export function PreciousMetalAssetSheet({ open, onClose, onSave }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    const g = parseFloat(grams), r = parseFloat(buyRate)
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 26, padding: '0 20px', flexShrink: 0 }}>
              <div style={{
                ...
                background: meta.bg, border: `1px solid ${meta.selBorder}`,
                ...
                <>
                      ...
                      {FORMS.map(f => (
                        <motion.button key={f} whileTap={{ scale: 0.92 }} onClick={() => setForm(f)}
                          style={{
                            ...
                            fontWeight:  form === f ? 700 : 400,
                            background:  form === f ? meta.bg : 'rgba(255,255,255,0.04)',
                            border:      form === f ? `1px solid ${meta.selBorder}` : '1px solid rgba(255,255,255,0.09)',
                            color:       form === f ? meta.accent : 'rgba(255,255,255,0.45)',
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 18 }}>
                    <label>
                      ...
                      <input type="number" inputMode="decimal" placeholder="0.000" value={grams} onChange={e => setGrams(e.target.value)}
                        style={{
                          width: '100%', padding: '13px 14px',
                          background: meta.bg, border: `1px solid ${meta.border}`,
                          borderRadius: 14, color: meta.accent, fontSize: 20, fontWeight: 800,
                          outline: 'none', fontVariantNumeric: 'tabular-nums', boxSizing: 'border-box' as const,
                        }}
                      />
                    </label>
                    <label>
                      <p style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)', marginBottom: 8 }}>Buy Rate (₹/g)</p>
                      <input type="number" inputMode="decimal" placeholder="0.00" value={buyRate} onChange={e => setBuyRate(e.target.value)}
                        style={{
                          width: '100%', padding: '13px 14px',
                          background: meta.bg, border: `1px solid ${meta.border}`,
                          borderRadius: 14, color: '#f5f7ff', fontSize: 20, fontWeight: 800,
                          outline: 'none', fontVariantNumeric: 'tabular-nums', boxSizing: 'border-box' as const,
                        }}
                      />
                    </label>
                  ...
                  <label style={{ display: 'block', marginBottom: 18 }}>
                    ...
                    <input type="text" placeholder="Jeweller, hallmark, locker location…" value={notes} onChange={e => setNotes(e.target.value)}
                      style={{
                        width: '100%', padding: '13px 16px',
                        background: meta.bg, border: `1px solid ${meta.border}`,
                        borderRadius: 14, color: '#f5f7ff', fontSize: 15,
              ...
              <div style={{
                ...
                <motion.button whileTap={{ scale: 0.97 }} onClick={handleSubmit} disabled={saving}
                  style={{
                    ...
                    background: saving ? meta.bg : meta.gradient,
                    border: 'none', borderRadius: 16,
                    color: saving ? meta.accent : '#fff', fontSize: 16, fontWeight: 800,
                    cursor: saving ? 'not-allowed' : 'pointer',
                    boxShadow: saving ? 'none' : `0 4px 20px ${meta.glow}`,
