...
export function RealEstateAssetSheet({ open, onClose, onSave }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSave({ label: `${assetName.trim()} \u2013 ${propertyType}`, category: 'Real Estate', value: parseFloat(Number(value).toFixed(2)), owner: 'Both', notes: notes.trim() })
      reset(); onClose()
    } catch (e) { setErr(e instanceof Error ? e.message : 'Failed to save') }
    finally { setSaving(false) }
  }

  const accent = '#fb923c', accentGlow = 'rgba(251,146,60,0.40)', accentBg = 'rgba(251,146,60,0.08)'
  const accentBorder = 'rgba(251,146,60,0.22)', accentSel = 'rgba(251,146,60,0.22)', accentSelBorder = 'rgba(251,146,60,0.55)'

  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div key="re-bd" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            onClick={handleClose}
            style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', zIndex: 40 }}
          />
          <motion.div key="re-sh" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
            transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }}
            style={{ position: 'fixed', bottom: 'var(--nav-h, 100px)', left: 0, right: 0, zIndex: 50, background: 'linear-gradient(180deg, #0f0a05 0%, #080604 100%)', border: `1px solid ${accentBorder}`, borderBottom: `1px solid rgba(251,146,60,0.10)`, borderRadius: '28px 28px 20px 20px', display: 'flex', flexDirection: 'column', maxHeight: 'calc(92dvh - var(--nav-h, 100px))' }}
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '16px 20px 8px' }}>
              <label style={{ display: 'block', marginBottom: 18 }}>
                ...
                <input type="text" placeholder="e.g. 3BHK Apartment, Whitefield Plot\u2026" value={assetName} onChange={e => setAssetName(e.target.value)}
                  style={{ width: '100%', padding: '13px 16px', background: accentBg, border: `1px solid ${accentBorder}`, borderRadius: 14, color: '#f5f7ff', fontSize: 16, outline: 'none' }}
                />
              </label>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {PROPERTY_TYPES.map(pt => (
                    <motion.button key={pt} whileTap={{ scale: 0.92 }} onClick={() => setPropertyType(pt)}
                      style={{ padding: '9px 16px', borderRadius: 100, fontSize: 13, fontWeight: propertyType === pt ? 700 : 400, background: propertyType === pt ? accentSel : 'rgba(255,255,255,0.04)', border: propertyType === pt ? `1px solid ${accentSelBorder}` : '1px solid rgba(255,255,255,0.09)', color: propertyType === pt ? accent : 'rgba(255,255,255,0.45)', cursor: 'pointer', transition: 'all 0.14s ease' }}
                    >{pt}</motion.button>
                  ))}
                </div>
              </div>
              <label style={{ display: 'block', marginBottom: 18 }}>
                ...
                <input type="number" inputMode="decimal" placeholder="0.00" value={value} onChange={e => setValue(e.target.value)}
                  style={{ width: '100%', padding: '13px 16px', background: accentBg, border: `1px solid ${accentBorder}`, borderRadius: 14, color: accent, fontSize: 20, fontWeight: 800, outline: 'none', fontVariantNumeric: 'tabular-nums' }}
                />
              </label>
              <label style={{ display: 'block', marginBottom: 20 }}>
                ...
                <input type="text" placeholder="Location, survey no., joint ownership\u2026" value={notes} onChange={e => setNotes(e.target.value)}
              ...
              {err && <p style={{ fontSize: 13, color: '#fca5a5', marginBottom: 8, padding: '10px 14px', background: 'rgba(248,113,113,0.1)', borderRadius: 10, border: '1px solid rgba(248,113,113,0.2)' }}>{err}</p>}
            </div>
