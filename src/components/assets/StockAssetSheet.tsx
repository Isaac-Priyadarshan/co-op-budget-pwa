...
const accent = '#22d3ee', accentGlow = 'rgba(34,211,238,0.35)', accentBg = 'rgba(34,211,238,0.07)'
const accentBorder = 'rgba(34,211,238,0.22)', accentSel = 'rgba(34,211,238,0.18)', accentSelBorder = 'rgba(34,211,238,0.55)'
...
export function StockAssetSheet({ open, onClose, onSave }: Props) {
  ...
  const reset = () => { setQuery(''); setResults([]); setSelected(null); setQty(''); setBuyPrice(''); setNotes(''); setErr(''); setSaving(false); setSearching(false) }
  ...
  const handleQueryChange = (val: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true)
      try { const r = await fetch(`/api/search-stock?q=${encodeURIComponent(val.trim())}`); const j = await r.json() as { results?: StockResult[] }; setResults(j.results ?? []) }
  ...
  const handleSubmit = async () => {
    ...
    const shares = parseFloat(qty), price = parseFloat(buyPrice), value = parseFloat((shares * price).toFixed(2))
    try {
      setSaving(true); setErr('')
      await onSave({ label: `${selected.name} (${selected.symbol})`, category: 'Stock', value, owner: 'Both', notes: notes.trim() || null, ticker: selected.symbol, quantity: shares, buy_price: price })
      reset(); onClose()
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.div key="st-sh" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
            transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }}
            style={{ position: 'fixed', bottom: 'var(--nav-h, 100px)', left: 0, right: 0, zIndex: 50, background: 'linear-gradient(180deg,#00080f 0%,#000608 100%)', border: `1px solid ${accentBorder}`, borderBottom: `1px solid rgba(34,211,238,0.10)`, borderRadius: '28px 28px 20px 20px', display: 'flex', flexDirection: 'column', maxHeight: 'calc(92dvh - var(--nav-h, 100px))' }}
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '0 20px 8px' }}>
              <div style={{ marginBottom: 16, position: 'relative' }}>
                ...
                <input type="text" placeholder="e.g. Reliance, HDFC Bank, Infosys\u2026" value={query} onChange={e => handleQueryChange(e.target.value)}
                ...
                {results.length > 0 && !selected && (
                  <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', marginTop: 4, background: '#0a0f14', border: `1px solid ${accentBorder}`, borderRadius: 14, overflow: 'hidden', zIndex: 60, maxHeight: 220, overflowY: 'auto' }}>
                    {results.map(s => (
            ...
            <div style={{ flexShrink: 0, padding: '12px 20px 16px', borderTop: `1px solid ${accentBorder}`, background: 'linear-gradient(180deg,#00080f 0%,#000608 100%)' }}>
