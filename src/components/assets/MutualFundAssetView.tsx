// src/components/assets/MutualFundAssetView.tsx
// Self-contained Mutual Fund asset module.
// Exports: MutualFundAssetCard
...
const accentSelBdr = 'rgba(167,139,250,0.55)'

// ─── MutualFundAssetCard ───────────────────────────────────────
export function MutualFundAssetCard({
  asset,
  reorderMode,
  dragHandleProps,
  onDelete,
      ...
      <div className="flex items-center gap-3 px-3.5 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            {asset.quantity != null && (
              <span className="text-[11px] text-white/35 tabular-nums">{asset.quantity} units</span>
            )}
            {asset.buy_price != null && (
              <span className="text-[11px] font-semibold tabular-nums" style={{ color: accent }}>
                @ ₹{asset.buy_price.toFixed(3)} NAV
              </span>
            )}
          </div>
        </div>
        <div className="text-right flex-shrink-0">
          <p className="text-[13px] font-extrabold tabular-nums m-0" style={{ color: currentVal !== null ? (pnlGain ? '#34d399' : '#f87171') : accent }}>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 px-3.5 pb-2 pt-1">
              ...
              <div className="text-center px-2 py-2 rounded-[10px]"
                ...
                <p className="text-[9px] uppercase tracking-widest text-white/35 m-0 mb-0.5">Units</p>
                <p className="text-[12px] font-extrabold tabular-nums m-0 text-[#f5f7ff]">{asset.quantity?.toFixed(3) ?? '—'}</p>
              </div>
              <div className="text-center px-2 py-2 rounded-[10px]"
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <p className="text-[9px] uppercase tracking-widest text-white/35 m-0 mb-0.5">Curr NAV</p>
