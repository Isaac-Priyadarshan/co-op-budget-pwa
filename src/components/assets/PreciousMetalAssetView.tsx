// src/components/assets/PreciousMetalAssetView.tsx
// Self-contained Precious Metal asset module.
// Exports: PreciousMetalAssetCard
...
const METAL_META: Record<Metal, { emoji: string; accent: string; bg: string; border: string; selBdr: string }> = {
  Gold:     { emoji: '🥇', accent: '#fbbf24', bg: 'rgba(251,191,36,0.07)',  border: 'rgba(251,191,36,0.22)',  selBdr: 'rgba(251,191,36,0.55)'  },
  Silver:   { emoji: '🥈', accent: '#94a3b8', bg: 'rgba(148,163,184,0.07)', border: 'rgba(148,163,184,0.22)', selBdr: 'rgba(148,163,184,0.55)' },
  Platinum: { emoji: '💎', accent: '#7dd3fc', bg: 'rgba(125,211,252,0.07)', border: 'rgba(125,211,252,0.22)', selBdr: 'rgba(125,211,252,0.55)' },
}
const FALLBACK = { emoji: '🏅', accent: '#e2e8f0', bg: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.12)', selBdr: 'rgba(255,255,255,0.25)' }

function detectMetal(label: string): Metal | null {
  if (/gold/i.test(label))     return 'Gold'
  if (/silver/i.test(label))   return 'Silver'
  if (/platinum/i.test(label)) return 'Platinum'
  return null
}

// ─── PreciousMetalAssetCard ────────────────────────────────────
export function PreciousMetalAssetCard({
  asset,
  reorderMode,
  dragHandleProps,
  onDelete,
  ...
  const metal = detectMetal(asset.label)
  const meta  = metal ? METAL_META[metal] : FALLBACK
    ...
    >
      <div className="flex items-center gap-3 px-3.5 py-3">
        {reorderMode ? (
          ...
          <span className="text-[22px] flex-shrink-0">{meta.emoji}</span>
        )}
          ...
          <div className="flex items-center gap-1.5">
            {asset.quantity != null && (
              <span
                ...
                style={{ background: meta.bg, border: `1px solid ${meta.selBdr}`, color: meta.accent }}
              >
                {asset.quantity}g
              </span>
            )}
        ...
        <div className="text-right flex-shrink-0">
          <p
            ...
            style={{ color: currentVal !== null ? (pnlGain ? '#34d399' : '#f87171') : meta.accent }}
          >
          ...
          >
            <div className="grid grid-cols-3 gap-2 px-3.5 pb-2 pt-1">
              <div className="text-center px-2 py-2 rounded-[10px]"
                ...
                <p className="text-[12px] font-extrabold tabular-nums m-0" style={{ color: meta.accent }}>{formatINR(invested)}</p>
              </div>
              <div className="text-center px-2 py-2 rounded-[10px]"
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <p className="text-[9px] uppercase tracking-widest text-white/35 m-0 mb-0.5">Weight</p>
                <p className="text-[12px] font-extrabold tabular-nums m-0 text-[#f5f7ff]">{asset.quantity ?? '—'}g</p>
              </div>
              <div className="text-center px-2 py-2 rounded-[10px]"
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <p className="text-[9px] uppercase tracking-widest text-white/35 m-0 mb-0.5">Live/g</p>
