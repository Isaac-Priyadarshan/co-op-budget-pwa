...
const accent      = '#a78bfa'  // violet — MF
const accentGlow  = 'rgba(167,139,250,0.35)'
...
export function MutualFundAssetSheet({ open, onClose, onSave }: Props) {
  ...
  const handleQueryChange = (val: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      setSearching(true)
      try {
        const r = await fetch(`https://api.mfapi.in/mf/search?q=${encodeURIComponent(val.trim())}`)
        const j = await r.json() as MFResult[]
        setResults(j.slice(0, 10))
      } catch { setResults([]) }
      finally { setSearching(false) }
    }, 400)
  }

  const handleSelect = async (mf: MFResult) => {
    setSelected(mf); setQuery(mf.schemeName); setResults([])
    // auto-fill buy NAV with current NAV
    try {
      const r = await fetch(`https://api.mfapi.in/mf/${mf.schemeCode}`)
      const j = await r.json() as { data?: { nav: string }[] }
      const nav = j.data?.[0]?.nav
      if (nav) setBuyNav(nav)
    } catch { /* leave blank */ }
  }
        ...
        <>
          ...
          <motion.div key="mf-sh"
            ...
            style={{
              ...
              background: 'linear-gradient(180deg,#0a0614 0%,#060408 100%)',
          ...
          >
            ...
            <div style={{ marginBottom: 16, position: 'relative' }}>
              ...
              <input type="text" placeholder="e.g. HDFC Flexi Cap, Mirae Asset, Axis…"
              ...
              {results.length > 0 && !selected && (
                <div style={{ position: 'absolute', left: 0, right: 0, top: '100%', marginTop: 4, background: '#0a0614', border: `1px solid ${accentBorder}`, borderRadius: 14, overflow: 'hidden', zIndex: 60, maxHeight: 280, overflowY: 'auto' }}>
                  {results.map(mf => (
                    <motion.button key={mf.schemeCode} whileTap={{ scale: 0.98 }}
                      onClick={() => handleSelect(mf)}
                      style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: '100%', padding: '12px 16px', background: 'none', border: 'none', borderBottom: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer' }}
                    >
                      <span style={{ fontSize: 13, color: '#f5f7ff', fontWeight: 600, textAlign: 'left', lineHeight: 1.3 }}>{mf.schemeName}</span>
                      <span style={{ fontSize: 11, color: accent, marginTop: 3 }}>Code: {mf.schemeCode}</span>
                    </motion.button>
                  ))}
                </div>
              )}
            ...
            {selected && (
              <div style={{ marginBottom: 18, padding: '10px 14px', background: accentSel, border: `1px solid ${accentSelBorder}`, borderRadius: 14 }}>
                <p style={{ fontSize: 13, color: '#f5f7ff', fontWeight: 600, margin: '0 0 3px', lineHeight: 1.3 }}>{selected.schemeName}</p>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <p style={{ fontSize: 11, color: accent, margin: 0 }}>Scheme Code: {selected.schemeCode}</p>
                  <button onClick={() => { setSelected(null); setQuery(''); setBuyNav('') }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.4)', fontSize: 16 }}>×</button>
                </div>
              </div>
            )}
            ...
            <label style={{ display: 'block', marginBottom: 24 }}>
              ...
              <input type="text" placeholder="AMC, folio number, SIP details…"
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.28)', marginBottom: 18, lineHeight: 1.5 }}>📡 NAV syncs daily at midnight IST via mfapi.in — P&amp;L auto-updates.</p>
