...
// ─── Delete helpers ───────────────────────────────────────────────────────────
const ALL_TABLES = ['transactions', 'wallets', 'loans', 'recurring', 'lent', 'borrowed', 'assets'] as const
...
async function eraseHistory(): Promise<void> {
  ...
  // Clear any local transaction caches
  for (const key of Object.keys(localStorage)) {
    if (key.startsWith('txn_') || key.startsWith('transactions_')) localStorage.removeItem(key)
  }
}

async function startFresh(year: number, month: number): Promise<void> {
  // month is 1-based. Build ISO range for the full calendar month.
...
async function fullWipeOut(): Promise<void> {
  // Delete all rows from every table in parallel.
  const SENTINEL = '00000000-0000-0000-0000-000000000000'
  await Promise.all(
    ALL_TABLES.map(table =>
      supabase.from(table).delete().neq('id', SENTINEL)
    )
  )
...
export function ResetDataSheet({ open, onClose }: Props) {
  ...
  // Start Fresh state
  const [sfMonth, setSfMonth] = useState<number>(new Date().getMonth() + 1) // 1-based
  const [sfYear,  setSfYear]  = useState<number>(CURRENT_YEAR)

  // Clear One Year state
  ...
  function isConfirmEnabled(): boolean {
    ...
    return true // erase_history — no extra input needed
  }
            ...
            <div style={{ padding: '8px 20px 32px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                <div>
                  ...
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', margin: '3px 0 0' }}>Select an option below</p>
                </div>
                <motion.button
                  whileTap={{ scale: 0.88 }}
                  onClick={handleClose}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{
                        width: 40, height: 40,
                        borderRadius: 12,
                        background: mode.bubbleBg,
                        border: `1px solid ${mode.bubbleBorder}`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              {/* ── Context UI (animates in based on selected mode) ── */}
              <AnimatePresence mode="wait">

                {/* Start Fresh — month + year pickers */}
                  ...
                  >
                    <div style={{
                      ...
                      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', margin: '8px 0 0' }}>
                        All transactions in {MONTHS[sfMonth - 1]} {sfYear} will be permanently deleted.
                      </p>
                    </div>
                  </motion.div>
                )}

                {/* Clear One Year — year dropdown */}
                ...
                {/* Full Wipe Out — WIPE confirmation input */}
                {selected === 'full_wipe' && (
                  <motion.div
                    key="ctx-full-wipe"
                    variants={contextVariants}
                  ...
                  >
                    <div style={{
                      ...
                      <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', margin: '0 0 12px', lineHeight: 1.5 }}>
                        This will permanently delete every transaction, wallet, loan, recurring payment, asset, lent and borrowed record — both in Supabase and local storage. This cannot be undone.
                      </p>
                  ...
                  >
                    <motion.button
                      ...
                      style={{
                        ...
                        transition: 'all 0.18s ease',
                        background: success
                          ? 'linear-gradient(135deg, rgba(20,184,120,0.8), rgba(20,184,100,0.6))'
                          : isFullWipe
                            ? (confirmEnabled
