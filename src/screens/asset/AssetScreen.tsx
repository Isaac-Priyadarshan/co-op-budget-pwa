        ...
        <>
          ...
          <motion.div key="blog-sh" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }} style={sheetShell}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 20px 18px', flexShrink: 0 }}>
              <div style={{ width: 44, height: 44, borderRadius: 14, background: 'rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>\u{1F4CB}</div>
              <div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' as any, padding: '0 20px 20px' }}>
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px 10px' }}>
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          ...
          {startDate && <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', margin: '3px 0 0', fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{fmtStartDate(startDate)}</p>}
        </div>
      </div>
      <AnimatePresence>
        {expanded && !reorderMode && (
          ...
          >
            <div style={{ display: 'flex', gap: 8, padding: '0 14px 13px', justifyContent: 'flex-end' }}>
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onLog(asset)}
              ...
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onEdit(asset)}
              ...
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onTopUp(asset)}
              ...
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onDelete(asset.id, asset.label)}
...
function GenericAssetCard({
  asset, reorderMode, dragHandleProps, onDelete, working, emoji,
}: {
  asset: AssetItem; reorderMode: boolean
  dragHandleProps?: React.HTMLAttributes<HTMLDivElement>
  onDelete: (id: string, label: string) => void; working: string | null; emoji: string
}) {
  const [expanded, setExpanded] = useState(false)
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -30, scale: 0.95 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }}
      style={{ borderRadius: 18, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', overflow: 'hidden', cursor: reorderMode ? 'grab' : 'pointer' }}
      onClick={() => { if (!reorderMode) setExpanded(e => !e) }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#f5f7ff', margin: '0 0 3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{asset.label}</p>
          {asset.notes && <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{asset.notes}</p>}
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <p style={{ fontSize: 14, fontWeight: 800, color: '#f5f7ff', fontVariantNumeric: 'tabular-nums', margin: 0 }}>{formatINR(asset.value)}</p>
        ...
        {expanded && !reorderMode && (
          ...
          >
            <div style={{ display: 'flex', gap: 8, padding: '0 14px 13px', justifyContent: 'flex-end' }}>
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onDelete(asset.id, asset.label)}
                disabled={working === asset.id}
                style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, color: '#f87171' }}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                  <polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2" />
                </svg>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px 10px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', fontVariantNumeric: 'tabular-nums' }}>{totalQty} shares</span>
            {asset.current_price != null && (
              <span style={{ fontSize: 11, fontWeight: 600, color: '#fcd34d', fontVariantNumeric: 'tabular-nums' }}>@ {formatINR(asset.current_price)}</span>
            )}
          </div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 5, justifyContent: 'flex-end' }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: '#fcd34d', fontVariantNumeric: 'tabular-nums' }}>
              {currentVal !== null ? formatINR(currentVal) : formatINR(totalInvested)}
            </span>
          </div>
          {pnlAbs !== null && pnlPct !== null && Math.abs(pnlAbs) >= 0.01 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 3, justifyContent: 'flex-end', marginTop: 3 }}>
              {pnlGain ? <ArrowUp color="#34d399" /> : <ArrowDown color="#f87171" />}
              <span style={{ fontSize: 10, fontWeight: 700, color: pnlGain ? '#34d399' : '#f87171', fontVariantNumeric: 'tabular-nums' }}>
      ...
      <AnimatePresence>
        {expanded && !reorderMode && (
          ...
          >
            <div style={{ display: 'flex', gap: 8, padding: '0 14px 13px', justifyContent: 'flex-end' }}>
              <motion.button whileTap={{ scale: 0.88 }} onClick={() => onLog(asset)}
                style={iconBtn('#fcd34d', 'rgba(251,191,36,0.1)', 'rgba(251,191,36,0.25)')}
                title="View log"
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" />
...
export default function AssetScreen() {
  ...
  if (activeGroup && activeGroupObj) {
    ...
    return (
      <div style={{ minHeight: '100dvh', background: '#070c16', padding: '0 0 100px' }}>
        <div style={{ padding: '56px 20px 0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => { setActiveGroup(null); setReorderMode(false) }}
              style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, color: '#f5f7ff' }}
            >
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="15 18 9 12 15 6" />
              </svg>
            </motion.button>
            <h1 style={{ fontSize: 22, fontWeight: 900, color: '#f5f7ff', margin: 0, letterSpacing: '-0.02em' }}>{activeGroupObj.emoji} {activeGroupObj.label}</h1>
            <motion.button whileTap={{ scale: 0.9 }} onClick={() => setReorderMode(r => !r)}
              style={{ marginLeft: 'auto', padding: '7px 14px', borderRadius: 10, background: reorderMode ? 'rgba(96,165,250,0.18)' : 'rgba(255,255,255,0.05)', border: reorderMode ? '1px solid rgba(96,165,250,0.4)' : '1px solid rgba(255,255,255,0.1)', color: reorderMode ? '#93c5fd' : 'rgba(255,255,255,0.45)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
            >{reorderMode ? 'Done' : 'Reorder'}</motion.button>
          </div>
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setAddSheet(true)}
            style={{ width: '100%', padding: '16px', background: `linear-gradient(135deg, ${activeGroupObj.border.replace('0.35','0.9')}, ${activeGroupObj.border.replace('0.35','0.6')})`, border: 'none', borderRadius: 16, color: '#fff', fontSize: 15, fontWeight: 800, cursor: 'pointer', boxShadow: `0 4px 20px ${activeGroupObj.color}` }}
          >+ Add {activeGroupObj.label} Asset</motion.button>
        </div>
    ...
    <div style={{ minHeight: '100dvh', background: '#070c16', padding: '0 0 100px' }}>
      <div style={{ padding: '56px 20px 0' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: '#f5f7ff', margin: '0 0 4px', letterSpacing: '-0.03em' }}>Assets</h1>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', margin: 0 }}>Track and manage your wealth</p>
        </div>

        <div style={{ marginBottom: 24 }}>
          <SummaryCard totalValue={totalValue} assetCount={assets.filter(a => !isTopUp(a.notes)).length} loading={loading} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {ASSET_GROUPS.map(group => (
            <GroupCard
              key={group.id}
              group={group}
              total={grouped[group.id]?.filter(a => !isTopUp(a.notes)).reduce((s, a) => s + a.value, 0) ?? 0}
