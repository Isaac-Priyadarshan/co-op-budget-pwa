...
export function LentScreen() {
  const { entries, loading, error, add, settle, remove, totalOwedToUs } = useLent()
  ...
  const handleSettle = async (id: string) => {
    setWorking(id); try { await settle(id) } catch (e) { console.error(e) } finally { setWorking(null) }
  }
    ...
    <div style={{ padding: '24px 20px 32px', minHeight: '100%' }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
          <div>
            <p style={{ fontSize: 12, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(110,231,183,0.7)', marginBottom: 4 }}>Others owe you</p>
        ...
        <div style={{ borderRadius: 20, padding: '18px 20px', background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', marginBottom: 20 }}>
          <p style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(110,231,183,0.6)', marginBottom: 6 }}>Total owed to you (unsettled)</p>
          <p style={{ fontSize: 32, fontWeight: 700, color: '#6ee7b7', letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>{formatINR(totalOwedToUs)}</p>
        </div>

        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          {(['pending', 'settled', 'all'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{ padding: '7px 16px', borderRadius: 100, border: filter === f ? '1px solid rgba(52,211,153,0.5)' : '1px solid rgba(255,255,255,0.08)', background: filter === f ? 'rgba(52,211,153,0.15)' : 'rgba(255,255,255,0.04)', color: filter === f ? '#6ee7b7' : 'rgba(255,255,255,0.45)', fontSize: 13, fontWeight: filter === f ? 600 : 400, cursor: 'pointer', textTransform: 'capitalize', transition: 'all 0.14s ease' }}
            >{f}</button>
          ))}
        </div>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 20px', borderRadius: 20, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 500, color: 'rgba(255,255,255,0.5)', marginBottom: 6 }}>{filter === 'pending' ? 'No pending lends!' : 'Nothing here yet'}</p>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)' }}>Tap + Add to record money you lent</p>
          </div>
        )}
          ...
          <AnimatePresence initial={false}>
            {filtered.map(entry => (
              <motion.div key={entry.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -40, scale: 0.95 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }}
                style={{ borderRadius: 18, padding: '16px', background: entry.settled ? 'rgba(255,255,255,0.02)' : 'rgba(52,211,153,0.07)', border: entry.settled ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(52,211,153,0.2)' }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>{entry.description} · {entry.lent_by} · {formatShortDate(entry.created_at)}</p>
                  </div>
                  <p style={{ fontSize: 18, fontWeight: 700, color: entry.settled ? 'rgba(255,255,255,0.3)' : '#6ee7b7', fontVariantNumeric: 'tabular-nums', flexShrink: 0, marginLeft: 12 }}>{formatINR(entry.amount)}</p>
                </div>
      ...
      <PersonEntrySheet open={sheetOpen} onClose={() => setSheetOpen(false)} mode="lent"
        onAdd={({ person, amount, description, actor }) => add({ person, amount, description, lent_by: actor })} />
    </div>
  )
}
